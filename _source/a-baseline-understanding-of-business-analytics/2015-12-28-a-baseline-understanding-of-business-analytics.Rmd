...
library(dplyr)
library(tidyr)
analytics.trend <- read.csv("/Volumes/Memorex/Business Analytics/analytics.trend.csv", stringsAsFactors=FALSE)
analytics.trend$Week <- substr(as.character(analytics.trend$Week), 1, 10)
analytics.trend$Week <- as.Date(analytics.trend$Week, "%Y-%m-%d")
analytics.trend <- analytics.trend %>%
        gather(discipline, value, -Week)

ggplot(analytics.trend, aes(Week, value, group=discipline, colour=discipline)) +
        geom_line() +
        theme_bw() +
...
<img src="https://d1avok0lzls2w.cloudfront.net/img_uploads/analytics_2(1).png" alt="Analytic Categories" align="middle" vspace="25" width="50%">
