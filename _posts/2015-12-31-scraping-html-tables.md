...
<a href="http://bradleyboehmke.github.io//2015/12/scraping-html-tables.html"><img src="https://analystatlarge.files.wordpress.com/2014/05/result.png" alt="Scraping HTML Tables" style="float:left; margin:0px 8px 0px 0px; width: 17%; height: 17%;"></a>
...
Remember that `html_nodes()` does not parse the data; rather, it [acts as a CSS selector](http://bradleyboehmke.github.io//2015/12/scraping-html-text.html#scraping_nodes). To parse the HTML table data we use `html_table()`, which would create a list containing 15 data frames.  However, rarely do we need to scrape *every* HTML table from a page, especially since some HTML tables don't catch any information we are likely interested in (i.e. table of contents, table of figures, footers). 
...
An alternative approach, which is more explicit, is to use the [element selector process described in the text scraping tutorial](http://bradleyboehmke.github.io//2015/12/scraping-html-text.html#specific_nodes) to call the table ID name. 
...
You can see that `tbls_xml` captures the same 15 `<table>` nodes that `html_nodes` captured. To capture the same tables of interest we previously discussed (*Table 2. Nonfarm employment...* and *Table 3. Net birth/death...*) we can use a couple approaches. First, we can assess `str(tbls_xml)` to identify the tables of interest and perform normal [list subsetting](http://bradleyboehmke.github.io//tutorials/list#subsetting). In our example list items 3 and 4 correspond with our tables of interest.
...
## Wrapping Up
Between `rvest` and `XML`, scraping HTML tables is relatively easy once you get fluent with the syntax and the available options.  This post covers just the basics of both these packages to get you moving forward with scraping tables. 
